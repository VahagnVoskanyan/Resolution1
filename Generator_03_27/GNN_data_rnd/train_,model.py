...
"""
1.Generates a set of random clauses (3 to 10 clauses).
2.Finds all resolvable pairs (just in a single pass, over every possible pair of clauses and their literals).
3.Scores each pair using your heuristic and chooses the single “best” pair.
4.Records that pair as the best resolution step for the dataset entry and then stops.
"""