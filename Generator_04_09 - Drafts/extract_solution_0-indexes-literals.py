    ...
    for problem_file in problem_files:
        ...
        for cid in clause_ids:
            ...
            if match:
                ...
                # NEW: Analyze resolved literals when we have both parents
                if len(clause_ids) == 2:  # Only for binary resolutions
                    other_cid = clause_ids[0] if cid == clause_ids[1] else clause_ids[1]
                    _, other_text = extract_clause_text(proof, other_cid)
                    
                    if other_text:
                        lit1, lit2 = find_resolved_literals(proof_txt, other_text, formula)
                        if lit1 and lit2:
                            print(f"  Resolved literals:\n"
                                  f"    From {cid}: {lit1}\n"
                                  f"    From {other_cid}: {lit2}")
                        else:
                            print("  Could not determine resolved literals")
            else:
